...
#define BMP_PARSE_PROGRESSIVE		101
#define BMP_PARSE_BMP				102
#define BMP_PARSE_GIF				103
#define BMP_PARSE_PNG				104
#define BMP_PARSE_FAIL				-1
#define BMP_SET_HW_ERROR			-2
...
//////////////////////////////////////////added by wwj, 20150525
typedef struct _st_FatSpeedUpNode
{
    struct _st_FatSpeedUpNode *pNext;
    INT32U StartCluster;
    INT32U ClusterCnt;
} st_FatSpeedUpNode, *p_st_FatSpeedUpNode;

typedef struct _st_FatSpeedUp
{
    INT8U             Flag;
    INT32U            BufferAddr;
    INT32U            BufferSize;
    INT32U            UsedCnt;
    p_st_FatSpeedUpNode pFreeList;
    p_st_FatSpeedUpNode pUsedList;
    p_st_FatSpeedUpNode pFirstUsedList;

} st_FatSpeedUp, *p_st_FatSpeedUp;
...
//define capture photo by piece
extern st_FatSpeedUp FatSpeedUp_Array[MAX_DISK_NUM];
...
INT32S FatSpeedUpInit(INT16S dsk, INT32U size);
INT32S UsrCheckFreeSize(INT16S dsk, INT32U size); //size is byte size
void UsrFlushBuffers(INT16S dsk);

INT32S write_start(INT16S fd, INT32U start_sector);
INT32S write_continue(INT16S fd, INT32U buf, INT32U size);
INT32S write_stop(INT16S fd);
INT32S get_sector_for_avi(INT16S fd, INT32U *pStartSector, INT32U *pSectorCnt, INT32U *pStartCluster);
INT32S get_next_sector_for_avi(INT16S fd, INT32U *pStartSector, INT32U *pSectorCnt, INT32U *pStartCluster);
INT32S UsrDelFreeCluster(INT16S fd, INT32U *ptrStartSectorBuf, INT32U *ptrSectorCntBuf, INT32U BlockCnt, INT32U *ptrIdxClusterBuf, INT32U IdxClusterCnt);
INT32S Photo_UsrDelFreeCluster(INT16S fd, INT32U StartSector, INT32U SectorCnt, INT32U *ptrIdxClusterBuf, INT32U IdxClusterCnt);

// Text Control Register Constatnt Definitions
#define	B_TFT_LB						24
#define	PPU_TFT_LONG_BURST				(((INT32U) 1)<<B_TFT_LB)
#define	B_CM_EN							23
#define	PPU_CM_ENABLE					(((INT32U) 1)<<B_CM_EN)
...
#define	B_SP_ROUND						3
#define	SP_ROUND_ROBIN_ENABLE			(((INT32U) 1)<<B_SP_ROUND)
...
extern INT32S gplib_ppu_color_mapping_set(PPU_REGISTER_SETS *p_register_set, INT32U enable, INT32U value);	// enable:0=disable color mapping 1=enable color mapping, value: 32-bit address of color mapping table
...
extern INT32S gplib_ppu_fifo_go_and_wait_done(void);
extern INT32S gplib_ppu_free_mode_size_rigister_set(INT16U INTL, INT16U H_size, INT16U V_size);
extern INT32S gplib_ppu_text_interpolation_mode_set(PPU_REGISTER_SETS *p_register_set, INT32U text_index, INT32U value);
extern INT32S gplib_ppu_text_rotate_zoom_register_set(INT32U text_index, INT16S angle, FP32 factor_k);
...
extern INT32S gplib_ppu_text_interpolation_mode_set(PPU_REGISTER_SETS *p_register_set, INT32U text_index, INT32U value);  // value:0=disable 1=enable
extern INT32S gplib_ppu_text_rotate_zoom_register_set(INT32U text_index, INT16S angle, FP32 factor_k);
...
extern INT32S gplib_ppu_large_sprite_enable_set(PPU_REGISTER_SETS *p_register_set, INT32U value);       // value:0=disable 1=enable
extern INT32S gplib_ppu_sprite_interpolation_enable_set(PPU_REGISTER_SETS *p_register_set, INT32U value);   // value:0=disable 1=enable
extern INT32S gplib_ppu_sprite_group_enable_set(PPU_REGISTER_SETS *p_register_set, INT32U value);         // value:0=disable 1=enable
extern INT32S gplib_ppu_sprite_extend_attribute_ram_ptr_set(PPU_REGISTER_SETS *p_register_set, INT32U value);// value: 32-bit pointer to sprite exterd attribute ram
...
#ifdef SUPPORT_5_1_CHANNELS
#define	AAC_DEC_MEMORY_BLOCK_SIZE	24948
#else
#define	AAC_DEC_MEMORY_BLOCK_SIZE	16688
#endif
...
void *VoiceChanger_Create(void);
void VoiceChanger_Del(void *pWorkMem);

// functions that used for memory allocated by User
#define VOICE_CHANGER_WORKMEM_SIZE	17124
int VoiceChanger_GetMemBlockSize(void);
void VoiceChanger_Initial(void *pWorkMem);
...
#define AVIPACKER_RESULT_CLUSTER_GRP_END		0x80000012  // wwj add
...
typedef enum
{
    AVIPACKER_MSG_STOP = 0x00010000,
    AVIPACKER_MSG_VIDEO_WRITE = 0x00020000,
    AVIPACKER_MSG_AUDIO_WRITE = 0x00030000,
    AVIPACKER_MSG_HDR_WRITE = 0x00040000
} AVIPACKER_MSG;
...
extern INT32S	SPU_Free_ToneColor_1(void);//free 第二组音色(free the second group tonecolor)
...
extern INT32S 	SPU_load_tonecolor_1(INT16S fd_idi, INT32U MIDI_Index, INT8U mode);//load第二组音色(load the second group tonecolor)
...
extern void SPU_Set_idi_addr(INT32U idi_addr);//(set "*.idi" data's start addr in nandflash(or in a file))设置idi数据在Nand flash中的起始地址或是在FS文件中的偏移地址
extern void SPU_Set_adpcm_comb_addr(INT32U adpcm_comb_addr, INT32U ram_buffer_addr);//设置adpcm_comb数据在Nand flash中的起始地址或是在FS文件中的偏移地址,以及存放ADPCM的临时RAM地址；ram_buffer_addr=0:调用user_memory_malloc()分配地址，其它，需要提供足够大的RAM区域的地址
...
extern INT32S SPU_Free_adpcm_data_temp_buffer(void);//(free adpcm temp buffer)释放用于存放ADPCM数据的内存
...
//mode:  0:NAND flash logic area, 2:NAND flash app area, 3:from file system
//--------------------------------------------------------------------------------------------------
//---------------------------------------------------------------------//切换音色和播音符用
extern INT32U	T_InstrumentStartSection_1[129];//切换音色和播音符用
extern INT32U	T_InstrumentPitchTable_1[500];//切换音色和播音符用
extern INT32U	*T_InstrumentSectionAddr_1[500];//切换音色和播音符用
//---------------------------------------------------------------------
...
// added by Bruce, 2008/10/31
//------------------------------------------------------------------------------
//#define _GP6_GRAPHIC_NORMAL_

#ifdef _GP6_GRAPHIC_NORMAL_
