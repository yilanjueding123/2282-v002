...
void gpio_pad_init2(void)
{
    ...
    funpos0 &= ~(1 << 9);
    funpos0 |= (1 << 8); /*(Fun_POS[9:8] == 2'h1)*/
#elif NAND_DATA5_0_POS==NAND_DATA5_0_AS_IOE5_0
    funpos0 |= (1 << 9);
    funpos0 &= ~(1 << 8); /*(Fun_POS[9:8] == 2'h2)*/
...
#error  XD DATA0~7 must ALL in IO8~15
        ...
        funpos0 |= (1 << 3);
        iosrsel &= ~(1 << 14); /*Nand CTRL Pin 設到 IOC6~9 */
#elif XD_CTRL_POS==XD_ALE_IOC12__CLE_IOC13__REB_IOC14__WEB_IOC15
        funpos0 &= ~(1 << 3);
        iosrsel &= ~(1 << 14); /*Nand CTRL Pin 設到 IOC12~15 */
#endif
