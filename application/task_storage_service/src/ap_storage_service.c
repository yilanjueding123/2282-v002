...
void ap_storage_service_init(void)
{
    ...
    sys_set_timer((void *)msgQSend, (void *)StorageServiceQ, MSG_STORAGE_SERVICE_STORAGE_CHECK, storage_mount_timerid, 64);
...
#ifdef GUEST_NAME_2822
		sprintf((char *)g_file_path, (const char *)"C:\\VIDEO\\REC0%04d.AVI", g_avi_index);
#else
...
#ifdef GUEST_NAME_2822
							sprintf((char *)f_name, (const char *)"REC0%04d.AVI", del_index);
#else
							sprintf((char *)f_name, (const char *)"MOVI%04d.AVI", del_index);
#endif
                            stat(f_name, &buf_tmp);
...
#ifdef GUEST_NAME_2822
		sprintf((char *)g_file_path, (const char *)"C:\\IMAGE\\JPG0%04d.JPG", g_jpeg_index);
...
#ifdef GUEST_NAME_2822	
	sprintf((char *)g_next_file_path, (const char *)"C:\\VIDEO\\REC0%04d.AVI", g_avi_index);
#else
...
/*****************************************************************************
 * 函 数 名  : get_file_final_avi_index
 * 负 责 人  : Zhibo
 * 创建日期  : 2017年5月9日
 * 函数功能  : 获取最后AVI文件的索引号
 * 输入参数  : void  NULL
...
#else
#ifdef GUEST_NAME_2822
        if (gp_strncmp((INT8S *) pdata, (INT8S *) "REC0", 4) == 0)
#else
...
#else
#ifdef GUEST_NAME_2822
		if (gp_strncmp((INT8S *) pdata, (INT8S *) "JPG0", 4) == 0)
#else
