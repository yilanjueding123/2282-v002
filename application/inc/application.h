...
typedef enum
{
    ...
    APP_STATES_RECORD,
    APP_STATES_CAPTURE,
    APP_STATES_CARD_DETE_SUC,
    APP_STATES_INIT_LED,
    APP_STATES_CARD_FULL,
    APP_STATES_WAIT_CORD,
    APP_STATES_WAIT_CAPTURE
} APP_STATES;
...
typedef struct
{
    ...
    INT8U   *string_ptr;
    INT32U	filenum;
    INT8U 	*Avi_Frame_buf1;    //添加buffer ptr 參數
...
typedef struct
{
    INT16S file_handle;			//文件句柄
    INT16S storage_free_size;	//剩余内存空间
    INT32U file_path_addr;		//文件路径
} STOR_SERV_FILEINFO;
