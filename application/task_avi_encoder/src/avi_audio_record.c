...
AVIPACKER_FRAME_INFO audio_frame[AVI_ENCODE_PCM_BUFFER_NO] =
{
	0
};
...
INT32S avi_adc_record_task_create(INT8U pori)
{
	...
	err 				= OSTaskCreate(avi_audio_record_entry, NULL,
		 (void *) &AviAudioRecordStack[C_AVI_AUDIO_RECORD_STACK_SIZE - 1], pori);
	...
	while (1)
	{
		...
		switch (msg_id)
		{
			case AVI_AUDIO_DMA_DONE:
				...
				{
					audio_frame[write_audio_idx].ext =
						 (AVI_AUDIO_PCM_SAMPLES << 1) / pAviEncPara->AviPackerCur->p_avi_wave_info->nBlockAlign;
					audio_frame[write_audio_idx].buffer_len =
						 ((((INT32U) (AVI_AUDIO_PCM_SAMPLES << 1) + 8 + 511) >> 9) << 9);
					audio_frame[write_audio_idx].is_used = 1;
					audio_frame[write_audio_idx].msg_id = AVIPACKER_MSG_AUDIO_WRITE;

					* (INT32S *) (audio_frame[write_audio_idx].buffer_addrs + (AVI_AUDIO_PCM_SAMPLES << 1) + 8) = (('J' & 0xFF) | (('U' & 0xFF) << 8) | (('N' & 0xFF) << 16) | (('K' & 0xFF) << 24));
													// add "JUNK"
					* (INT32S *) (audio_frame[write_audio_idx].buffer_addrs + (AVI_AUDIO_PCM_SAMPLES << 1) + 12) =
						 audio_frame[write_audio_idx].buffer_len - (AVI_AUDIO_PCM_SAMPLES << 1) - 16; // add length
			...
			case AVI_AUDIO_RECORD_START:
...

				{
					...
					for (i = 0; i < AVI_ENCODE_PCM_BUFFER_NO; i++)
					{
						...
						* (INT32S *) (audio_frame[i].buffer_addrs) = (('0' & 0xFF) | (('1' & 0xFF) << 8) | (('w' & 0xFF) << 16) | (('b' & 0xFF) << 24));
													// add "01wb"
